...
if (isset($_POST['reset-request-submit'])) {
  ...
  $url = 'https://'.$configs['installdirectory'].'change-password.php?selector=' . $selector . '&validator=' . bin2hex($token);
  ...
  $headers = 'From: '.$configs['sitename'].' <admin@'.$configs['site'].'>\r\n';
  $headers = 'Reply-To: admin@'.$configs['site'].'\r\n';
  $headers = 'Content-type: text/html\r\n';

  mail($to, $subject, $message, $headers);

  header('Location: ../change-password.php?reset=success');
